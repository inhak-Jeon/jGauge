...
void CjGaugeDlg::ChangeCamState()
{
	if (m_isStopCam)
	{
		...
		m_btnCapture.SetWindowTextW(CString("■"));
	}
	else {
		m_cam->stopGrabbing();
		m_btnCapture.SetWindowTextW(CString("▶"));
...
HBRUSH CjGaugeDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
	...
	if (nCtlColor == CTLCOLOR_STATIC || nID == IDC_STATIC_INFO_PLATE_R || nID == IDC_STATIC_INFO_FLAG_L || nID == IDC_STATIC_INFO_FLAG_R)	//STATIC 컨트롤만 배경색 변경
	{
		pDC->SetBkMode(OPAQUE);
		pDC->SetBkColor(RGB_BACK);	//배경색 변경
	}
	else {
		//그 외의 컨트롤은 배경을 투명하게	
	...
	{
		pDC->SetBkMode(OPAQUE);
		pDC->SetBkColor(RGB(255, 0, 0));	//배경색 변경
	}
